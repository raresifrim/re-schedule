...
use solana_runtime::{
    bank::Bank, bank_forks::BankForks, runtime_config::RuntimeConfig,
...
pub fn load_bank_from_snapshot(
    ...
    if !snapshot_path.exists() {
        panic!(
            "Snapshot archive path provided in the config.json does not exist. Please update it with an exisitng location or download a new snapshot using the download-snapshot command"
        );
    }
    ...
    let snapshot_bank = Bank::new_from_parent(
        snapshot_bank.clone(),
        &Pubkey::new_unique(),
        snapshot_bank.slot() + 1,
    );
