...
impl<Tx> Issuer<Tx> for BloomCounterIssuer 
            ...
            match selected_worker {
                ...
                Ok(operation) => {
                    ...
                    match operation.recv(&completed_work_receiver[worker_index]) {
                        Ok(finished_work) => {
                            let mut guard = self.conflict_families[worker_index].write().unwrap();
                            if finished_work.completed_entry.is_some() {
                                ...
                                for harness_tx in completed_work {
                                    ...
                                    for pair in write_account_locks.iter().zip_longest(read_account_locks.iter()){
                                        match pair {
                                            itertools::EitherOrBoth::Both(write_account, read_account ) => {
                                                let index = self.hasher.hash_one(write_account);
                                                guard.write_filter.query_and_dec_u64(index);
                                                let index = self.hasher.hash_one(read_account);
                                                guard.read_filter.query_and_dec_u64(index);
                                            },
                                            itertools::EitherOrBoth::Left(write_account) => {
                                                let index = self.hasher.hash_one(write_account);
                                                guard.write_filter.query_and_dec_u64(index);
                                            },
                                            itertools::EitherOrBoth::Right(read_account) => {
                                                let index = self.hasher.hash_one(read_account);
                                                guard.read_filter.query_and_dec_u64(index);
                                            }
                                        }
